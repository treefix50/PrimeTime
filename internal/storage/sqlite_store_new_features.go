...
func (s *Store) CreateTranscodingProfile(profile server.TranscodingProfile) error {
	...
	_, err := s.db.Exec(`
		INSERT INTO transcoding_profiles (
			...
			audio_layout, audio_normalization, preferred_languages, resolution, max_bitrate, container, created_at
		)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, profile.ID, profile.Name, profile.VideoCodec, profile.AudioCodec,
		...
		nullString(profile.AudioNormalization),
...
func (s *Store) GetTranscodingProfile(id string) (*server.TranscodingProfile, bool, error) {
	...
	var audioNormalization sql.NullString
	var createdAt int64
	...
	`, id).Scan(&profile.ID, &profile.Name, &profile.VideoCodec, &profile.AudioCodec,
		&supportedAudioCodecs, &maxAudioChannels, &audioLayout, &audioNormalization, &preferredLanguages,
	...
	if audioNormalization.Valid {
		profile.AudioNormalization = audioNormalization.String
	}
